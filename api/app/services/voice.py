...
def tts(
    ...
    # Use Google TTS (gTTS) as primary provider - free, no API keys, works globally
...
def gtts_tts(
    ...
    Free, no API keys required, works globally.
    """
    try:
        ...
        # gTTS doesn't support speed/rate parameter in the same way
        # but we can use it for language selection
        # Extract language from voice_name if provided (e.g., "en-US-AvaMultilingualNeural" -> "en")
        language = "en"  # Default to English
        if "-" in voice_name:
            lang_part = voice_name.split("-")[0]
            if lang_part in ["en", "es", "fr", "de", "it", "pt", "ru", "zh", "ja", "ko"]:
                language = lang_part
        ...
        # Create a basic SubMaker for compatibility
        # Since gTTS doesn't provide timing info, we create basic subtitles
        sub_maker = edge_tts.SubMaker()
        
        # Estimate timing: split text into words and distribute across audio duration
        words = text.split()
        total_words = len(words)
        
        # Rough estimation: ~140-160 words per minute for English speech
        # So duration ≈ (words / 150) * 60 seconds
        estimated_duration_ms = int((total_words / 150) * 60 * 1000)
        ms_per_word = estimated_duration_ms // total_words if total_words > 0 else 1000
        ...
        for i, word in enumerate(words):
            ...
            sub_maker.create_sub((start_ms, ms_per_word), word)
            current_ms = end_ms
...
def gemini_tts(
    ...
    """
    Gemini TTS is currently disabled due to API compatibility issues.
    Users selecting Gemini voices will automatically fallback to SiliconFlow TTS.
    """
    logger.warning(f"Gemini TTS is disabled. Fallback to SiliconFlow TTS.")
    return None
