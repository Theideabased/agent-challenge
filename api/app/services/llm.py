...
def _generate_response(prompt: str, gemini_api_key: str = "") -> str:
    try:
        content = ""
        ...
        if llm_provider == "g4f":
            ...
            if llm_provider == "moonshot":
                ...
                # Use user-provided API key if available, otherwise use config
                api_key = gemini_api_key if gemini_api_key and gemini_api_key.strip() else config.app.get("gemini_api_key")
                ...
                if gemini_api_key and gemini_api_key.strip():
                    ...
                    logger.warning(f"⚠️ Using config Gemini API key (no user key provided)")
...
def generate_script(
    video_subject: str, language: str = "", paragraph_number: int = 1, gemini_api_key: str = ""
) -> str:
    prompt = f"""
...
""".strip()
    ...
    for i in range(_max_retries):
        try:
            response = _generate_response(prompt=prompt, gemini_api_key=gemini_api_key)
            if response:
